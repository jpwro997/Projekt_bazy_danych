...
BEGIN
	IF (NEW.Nr_pojazdu IN 
		(SELECT Nr_Pojazdu FROM Wykaz_sluzb WHERE Data_sluzby=NEW.Data_sluzby))
	THEN
...
CREATE VIEW Miesieczne_wynagrodzenia AS(
	SELECT Rok_miesiac,ID_kierowcy,Stawka_podstawowa+sign((extract(epoch from Msc_suma_godzin-'168:00:00')+1*(1/2))::integer)*(Msc_suma_godzin-'168:00:00')*Stawka_za_nadgodziny AS Wyplata
...
RETURNS VOID AS $$
BEGIN
	DELETE FROM Kierowcy WHERE ID=ID_zwalnianego;
END;
$$ LANGUAGE 'plpgsql';
